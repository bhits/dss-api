@echo off
echo 'Will increase Minor parts of version numbers in pom.xml files by 1'
...
::
:: increase the minor version to get the new version
::
echo.-- increase the minor version to get the new version
set /a sum1=%minor%+1
set new_minor=%sum1%
...
git commit -a -m "Increase minor parts of version numbers in pom.xml files by 1"
